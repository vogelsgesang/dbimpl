...
namespace dbImpl {
  ...
  std::vector<RelationSchema> SchemaParser::parse(std::istream& in) {
     std::string token;
     unsigned line=1;
     std::vector<RelationSchema> s;
     while (in >> token) {
  ...
  void SchemaParser::nextToken(unsigned line, const std::string& token,
    ...
    switch(state) {
      ...
      case State::Table:
        if (isIdentifier(tok)) {
          ...
          schema->push_back(RelationSchema(token));
        } else {
      ...
      case State::CreateTableBegin:
        if (tok.size()==1 && tok[0]==literal::ParenthesisRight) {
          ...
          schema->back().attributes.push_back(AttributeDescriptor());
          schema->back().attributes.back().name = token;
      ...
      case State::KeyListBegin:
        if (isIdentifier(tok)) {
          struct AttributeNamePredicate {
            ...
            bool operator()(const AttributeDescriptor& attr) const {
              return attr.name == name;
            }
          };
          const auto& attributes = schema->back().attributes;
          AttributeNamePredicate p(token);
          auto it = std::find_if(attributes.begin(), attributes.end(), p);
          if (it == attributes.end())
            throw ParserError(line, "'"+token+"' is not an attribute of '"+schema->back().name+"'");
          schema->back().primaryKey.push_back(std::distance(attributes.begin(), it));
          state=State::KeyName;
        } else {
      ...
      case State::AttributeName:
        if (tok==keyword::Integer) {
          schema->back().attributes.back().type=TypeTag::Integer;
          state=State::AttributeTypeInt;
        } else if (tok==keyword::Char) {
          schema->back().attributes.back().type=TypeTag::Char;
      ...
      case State::CharBegin:
        if (isInt(tok)) {
          schema->back().attributes.back().len=std::atoi(tok.c_str());
      ...
      case State::Not:
        if (tok==keyword::Null) {
          schema->back().attributes.back().notNull=true;
          state=State::Null;
        }
